...
function computeTrends(json) {
  ...
  const percentagePreviousDay = numeral(1 - yesterday / thedaybefore).format('0%');
  ...
  const past7daysAvg = numeral(past7days.reduce((a, b) => (a + b)) / past7days.length).format('0.0');
  const next7daysAvg = numeral(next7days.reduce((a, b) => (a + b)) / next7days.length).format('0.0');
  const percentage7days = numeral(1 - next7daysAvg / past7daysAvg).format('0%');
